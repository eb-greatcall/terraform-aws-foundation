...
resource "aws_iam_role_policy_attachment" "master-node-attach-ebs-volume" {
  ...
  policy_arn = element(
    module.master-node-ebs-volumes.iam_volume_policy_arns,
    count.index,
  )
}
...
resource "aws_launch_configuration" "master-node-lc" {
  ...
  iam_instance_profile = element(
    aws_iam_instance_profile.master-node-iam-profile.*.id,
    count.index,
  )
  ...
  security_groups = concat([aws_security_group.transport-sg.id], var.extra_sg_ids)
...
data "template_file" "master-node-setup" {
  ...
  vars = {
    mount_snippet = element(
      module.master-node-ebs-volumes.volume_mount_snippets,
      count.index,
    )
    device_name           = "/dev/xvdf"
    ...
    config_yaml = element(
      data.template_file.master-node-config.*.rendered,
      count.index,
    )
